...
from src.model.battery import FEASIBILITY_TOL, BatterySpec, simulate_schedule
...
def test_power_and_soc_bounds_are_respected_on_volatile_prices():
    ...
    result = solve_dispatch(prices, SPEC)
    trace = result.trace
    assert trace["charge_mw"].max() <= SPEC.power_mw + FEASIBILITY_TOL
    assert trace["discharge_mw"].max() <= SPEC.power_mw + FEASIBILITY_TOL
    assert trace["soc_mwh"].min() >= SPEC.soc_min_mwh - FEASIBILITY_TOL
    assert trace["soc_mwh"].max() <= SPEC.soc_max_mwh + FEASIBILITY_TOL
